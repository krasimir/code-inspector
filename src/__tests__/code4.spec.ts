  ...
  describe('when analyzing the code', () => {
    it('should return all the scope nodes', () => {
      ...
      expect(expectation).toStrictEqual([
        'Program | Program',
        'ArrowFunctionExpression | getIcon(type:ItemType)',
        'FunctionDeclaration | trimText(str:string, n:number)',
        'FunctionDeclaration | arrayMove(arr:Array, oldIndex:number, newIndex:number)',
        'FunctionDeclaration | createGraphItem(entity:Entity, parent:string | undefined)',
        'FunctionDeclaration | addMoreIcon(e:Entity)',
        ...
        'FunctionDeclaration | createGraphItems(es:Entity[] | undefined, parent?:string | undefined)',
